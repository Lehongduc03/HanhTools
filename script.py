...
# Nếu FALSE: element có insulation rồi sẽ bị bỏ qua.
...
def get_insulation_existing(elem):
    ...
    for dep_id in dep_ids:
        try:
            ...
            if "Insulation" in dep.GetType().Name:
                return "YES"
        except:
            pass
...
def create_insulation_for_element(revit_doc, elem, rule):
    ...
    if thickness_mm is None or thickness_mm <= 0:
        return False, "Invalid ThicknessMM"

    if get_insulation_existing(elem) == "YES" and not REPLACE_EXISTING_INSULATION:
        return False, "Existing insulation skipped"

    insulation_type = get_insulation_type_by_name(revit_doc, elem_type, rule.InsulationTypeName)

    if insulation_type is None:
        return False, "Insulation type not found: {0}".format(rule.InsulationTypeName)
    ...
    def external_apply_rules(self, revit_doc, active_view, active_uidoc):
        ...
        try:
            ...
            for elem in elems:
                ...
                ok, msg = create_insulation_for_element(revit_doc, elem, rule)

                if ok:
                    created += 1
                else:
                    skipped += 1
                    messages.append("Element {0}, Rule {1}: {2}".format(
        ...
        alert("Apply Rules done.\nCreated: {0}\nSkipped: {1}\nSee pyRevit output.".format(created, skipped))
